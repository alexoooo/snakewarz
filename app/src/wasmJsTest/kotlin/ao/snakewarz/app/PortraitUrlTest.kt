 ...
 * with an entry below and no `.svg` beside the page is a broken image, and the browser is what says
 * so.
 */
class PortraitUrlTest {
    ...
    fun `and nothing is shipped for a bot no registry offers`() {
        ...
        assertEquals(seats(), SHIPPED_PORTRAITS)
    }
    ...
    fun `the address is relative, because the site is served out of a subdirectory`() {
        ...
        assertEquals("portrait/uct.svg", portraitUrl("uct"))
    }
