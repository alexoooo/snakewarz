...
internal class SlotForm(
    ...
    fun read(): SlotOptions? {
        ...
        for (row in rows) {
            when (val knob = row.knob) {
                is BotKnob.Search -> budget = allowanceOf(row, knob)
                is BotKnob.Param<*> -> valueOf(row, knob)?.let { params[knob.name] = it }
            }
        }
